    ...
    public class DbContext
    {
        ...
        private void InitializeDbSets()
        {
            foreach (var dbSet in this.dbSetProperties)
            {
                ...
                var dbSetPropety = dbSet.Value;

                var populateDbSetGeneric = typeof(DbContext)
                    .GetMethod("PopulateDbSet", BindingFlags.Instance | BindingFlags.NonPublic)
                    .MakeGenericMethod(dbSetType);

                populateDbSetGeneric.Invoke(this, new object[] { dbSetPropety });
        ...
        private void MapAllRealtions()
        {
            foreach (var dbSetProperty in this.dbSetProperties)
            {
                ...
                mapRealtionsgeneric.Invoke(this, new[] { dbSet });
            }
        }

        private void MapRealtions<TEnity>(DbSet<TEnity> dbSet)
            where TEnity : class, new()
        {
            var entityType = typeof(TEnity);
